...
import { TypeDetailsApiSchema } from '@repo/types';
import { fetchJson, POKEAPI_BASE_URL } from '@/features/pokemon/utils/fetcher';
import { cacheGet, cacheSet } from '@/lib/cache';
import { logger } from '@/lib/logger';
import { mapTypeDetails } from './mapper';

export async function fetchTypeDetails(typeName: string): Promise<TypeData | undefined> {
    const cacheKey = `type:${typeName}`;
    const cached = cacheGet<TypeData>(cacheKey);
    if (cached !== undefined) {
        return cached;
    }
